...
BATCH_SIZE = 50
_GEMINI_DIM = 3072 #gemini embedding dimension
_FALLBACK_DIM = 768  #fallback model is all-mpnet-base-v2
_active_model = None
_model_type: str | None = None  # "gemini" or "fallback"

# Model initialisation 
def _probe_gemini():
    #tries sending an embed query to gemini embedding model, it is like health check
    try:
...
def get_embedding_dim() -> int:
    # returns embedding model dimensions, just for avoiding access to sensitive variables
    _init()
    return _GEMINI_DIM if _model_type == "gemini" else _FALLBACK_DIM

#Batch embedding with retry , returns a batch of text embedded
...
def embed_texts(texts: list[str]) -> list[list[float]]:
    #to embed all the multiple documents/chunks. each doc/chunk has multiple batches in it
    _init()
